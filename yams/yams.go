...
func ReadProperty(filename string, path string) (interface{}, error) {
	data, err := readPathInFile(filename, path)
	if err != nil {
		return "", err
	}
	return data, nil
...
func readPathInFile(filename string, path string) (interface{}, error) {
	var parsedData yaml.MapSlice

	if err := readData(filename, &parsedData); err != nil {
		var generalData interface{}
		if err = readData(filename, &generalData); err != nil {
			return nil, err
		}
		item := yaml.MapItem{Key: "thing", Value: generalData}
		parsedData = yaml.MapSlice{item}
	...
	if parsedData != nil && parsedData[0].Key == nil {
		var interfaceData []map[interface{}]interface{}
		if err := readData(filename, &interfaceData); err == nil {
			var listMap []yaml.MapSlice
			for _, item := range interfaceData {
				listMap = append(listMap, mapToMapSlice(item))
			}
...
func writeFileInPlace(filename string, updatedData interface{}) error {
	dataStr, err := yamlToString(updatedData)
	if err != nil {
		return err
	}
...
func updateYamlScript(filepath string, key string, value string) (interface{}, error) {
	...
	if err := readData(filepath, &parsedData); err != nil {
		var generalData interface{}
		if err = readData(filepath, &generalData); err != nil {
			return nil, err
		}
		item := yaml.MapItem{Key: "thing", Value: generalData}
		parsedData = yaml.MapSlice{item}
